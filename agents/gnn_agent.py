...
class GNNAgent(Agent):
    def __init__(self, input_dim, config):
        ...
        # --- MODIFIED: Ensure learning rate is a float ---
        learning_rate = float(self.rl_cfg.get('learning_rate', 0.001))
        self.optimizer = optim.Adam(self.policy_network.parameters(), lr=learning_rate)
        self.gamma = self.rl_cfg.get('gamma', 0.99)
    ...
    def load_model(self, path):
        # --- MODIFIED: Load both model and optimizer states ---
        checkpoint = torch.load(path)
        self.policy_network.load_state_dict(checkpoint['model_state_dict'])
        self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        
    def save_model(self, path):
        # --- MODIFIED: Save both model and optimizer states ---
